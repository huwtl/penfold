...
class JobTest extends Specification {
  ...
  "create new future job" in {
    val createdJob = Job.create(AggregateId("1"), Binding(List(BoundQueue(queue))), DateTime.now().plusHours(1), Payload(Map()))
  ...
  "ensure only waiting jobs can be triggered" in {
    ...
    Job.create(AggregateId("1"), Binding(List(BoundQueue(queue))), Payload(Map())).start(queue).trigger() must throwA[RuntimeException]
  }

  "start job" in {
    val startedJob = Job.create(AggregateId("1"), Binding(List(BoundQueue(queue))), Payload(Map())).start(queue)
  ...
  "ensure only ready jobs can be started" in {
    Job.create(AggregateId("1"), Binding(List(BoundQueue(queue))), DateTime.now().plusHours(1), Payload(Map())).start(queue) must throwA[RuntimeException]
    Job.create(AggregateId("1"), Binding(List(BoundQueue(queue))), Payload(Map())).start(queue).start(queue) must throwA[RuntimeException]
  }

  "cancel job" in {
    val startedJob = Job.create(AggregateId("1"), Binding(List(BoundQueue(queue))), Payload(Map())).cancel(queue)
    typesOf(startedJob.uncommittedEvents) must beEqualTo(List(classOf[JobCancelled], classOf[JobTriggered], classOf[JobCreated]))
  }

  "complete job" in {
    val startedJob = Job.create(AggregateId("1"), Binding(List(BoundQueue(queue))), Payload(Map())).start(queue).complete(queue)
  ...
  "ensure only started jobs can be completed" in {
    ...
    Job.create(AggregateId("1"), Binding(List(BoundQueue(queue))), Payload(Map())).start(queue).cancel(queue).complete(queue) must throwA[RuntimeException]
  }
