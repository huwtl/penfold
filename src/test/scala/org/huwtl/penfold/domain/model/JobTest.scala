...
class JobTest extends Specification {
  ...
  "create new job" in {
    val createdJob = Job.create(Id("1"), QueueName(""), Payload(Map()))
    typesOf(createdJob.uncommittedEvents) must beEqualTo(List(classOf[JobCreated]))
  }

  "create new future job" in {
    val createdJob = Job.create(Id("1"), QueueName(""), DateTime.now(), Payload(Map()))
    typesOf(createdJob.uncommittedEvents) must beEqualTo(List(classOf[JobCreated]))
  }

  "trigger job" in {
    val triggeredJob = Job.create(Id("1"), QueueName(""), Payload(Map())).trigger()
  ...
  "ensure only waiting jobs can be triggered" in {
    Job.create(Id("1"), QueueName(""), Payload(Map())).trigger().trigger() must throwA[RuntimeException]
    Job.create(Id("1"), QueueName(""), Payload(Map())).trigger().start().trigger() must throwA[RuntimeException]
  }

  "start job" in {
    val startedJob = Job.create(Id("1"), QueueName(""), Payload(Map())).trigger().start()
  ...
  "ensure only triggered jobs can be started" in {
    Job.create(Id("1"), QueueName(""), Payload(Map())).start() must throwA[RuntimeException]
    Job.create(Id("1"), QueueName(""), Payload(Map())).trigger().start().start() must throwA[RuntimeException]
  }

  "cancel job" in {
    val startedJob = Job.create(Id("1"), QueueName(""), Payload(Map())).trigger().cancel()
    typesOf(startedJob.uncommittedEvents) must beEqualTo(List(classOf[JobCancelled], classOf[JobTriggered], classOf[JobCreated]))
  }

  "complete job" in {
    val startedJob = Job.create(Id("1"), QueueName(""), Payload(Map())).trigger().start().complete()
  ...
  "ensure only started jobs can be completed" in {
    Job.create(Id("1"), QueueName(""), Payload(Map())).complete() must throwA[RuntimeException]
    Job.create(Id("1"), QueueName(""), Payload(Map())).trigger().complete() must throwA[RuntimeException]
  }
