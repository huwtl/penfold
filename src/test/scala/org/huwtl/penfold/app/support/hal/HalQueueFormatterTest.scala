...
class HalQueueFormatterTest extends Specification {
  ...
  def task(id: String, binding: QueueBinding = QueueBinding(queueId)) = {
    TaskRecord(AggregateId(id), AggregateVersion.init, createdDate, binding, status, createdDate, triggerDate, triggerDate.getMillis, triggerDate.getMillis, Payload.empty)
  }
}
