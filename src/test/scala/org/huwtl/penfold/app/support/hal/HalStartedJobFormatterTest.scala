...
class HalStartedJobFormatterTest extends Specification {
  ...
  "format started job feed entry as hal+json" in {
    val job = JobRecord(Id("1"), DateTime.now, JobType(""), Status.Started, DateTime.now, Payload(Map()))
  ...
  "format started job feed as hal+json" in {
    val job1 = JobRecord(Id("1"), DateTime.now, JobType(""), Status.Started, DateTime.now, Payload(Map()))
    val job2 = JobRecord(Id("2"), DateTime.now, JobType(""), Status.Started, DateTime.now, Payload(Map()))
