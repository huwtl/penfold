...
class HalCompletedJobFormatterTest extends Specification {
  ...
  "format completed job feed entry as hal+json" in {
    val job1 = JobRecord(Id("1"), DateTime.now, JobType(""), Status.Completed, DateTime.now, Payload(Map()))
  ...
  "format completed job feed as hal+json" in {
    val job1 = JobRecord(Id("1"), DateTime.now, JobType(""), Status.Completed, DateTime.now, Payload(Map()))
    val job2 = JobRecord(Id("2"), DateTime.now, JobType(""), Status.Completed, DateTime.now, Payload(Map()))
