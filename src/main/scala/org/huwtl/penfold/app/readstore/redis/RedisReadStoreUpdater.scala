...
class RedisReadStoreUpdater(redisClientPool: RedisClientPool, objectSerializer: ObjectSerializer, keyFactory: RedisKeyFactory) extends EventListener {
  private val dateFormatter = DateTimeFormat.forPattern("yyyy-MM-dd HH:mm:ss")

  private val eventTrackerKey = keyFactory.eventTrackerKey("readstore")
