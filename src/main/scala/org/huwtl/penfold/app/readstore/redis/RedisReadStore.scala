...
import scala.Some
import org.huwtl.penfold.readstore.JobRecordReference
...
class RedisReadStore(redisClientPool: RedisClientPool, indexes: Indexes, objectSerializer: ObjectSerializer, keyFactory: RedisKeyFactory) extends ReadStore {
  ...
  override def checkConnectivity: Either[Boolean, Exception] = {
    try {
      redisClientPool.withClient(_.get("1"))
